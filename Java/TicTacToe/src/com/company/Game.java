...
public class Game {
    ...
    public void newGame( boolean twoPlayer ) {
        ...
        //initialize all charts in 3x3 game grid to '-'
        grid = new char[3][3];
    ...
    public boolean playAt(int i, int j){
        ...
        if(this.grid[i][j] != '-')
            return false; //bail put if not available
    ...
    public void nextTurn()
    {
        ...
        if(!this.twoPlayer){
            ...
            } while(this.grid[ai_i][ai_j] != '-');//keep trying if this spot was taken
    ...
    public boolean doChecks(){
        ...
        String winnerMessage = checkGameWinner(this.grid);
        if(!winnerMessage.equals("None")){
            this.gui.gameOver(winnerMessage);
            newGame(false);
            return true;
        }
        return false;
     ...
     */
    private String checkGameWinner( char[][] grid ) {
        String result = "None";
        for(int i = 0; i < 3; i++) {
        ...
        if (grid[0][2] == grid[1][1] && grid[1][1] == grid[2][0] && grid[2][0] != '-')
            if(grid[0][2] == 'x')
                ...
                result = "O wins";
        if(result == "None" && this.freeSpots == 0)
            result = "Tie";
        return result;
    }
